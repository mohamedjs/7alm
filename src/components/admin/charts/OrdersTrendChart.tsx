...
const SERIES = [
  { key: "retail" as const, label: "Retail (website)", color: seriesColors.retail },
  { key: "social" as const, label: "Social media", color: seriesColors.social },
];
...
export default function OrdersTrendChart({ data }: OrdersTrendChartProps) {
  ...
  // Direct end labels: nudge apart when the two lines converge at the edge.
  const endLabels = useMemo(() => {
    ...
    return [
      { text: "Retail", y: retailY, color: seriesColors.retail },
      { text: "Social", y: socialY, color: seriesColors.social },
    ];
  }, [data, plot]);
  ...
  return (
    <div>
      ...
      <div
        ...
        tabIndex={0}
        role="img"
        aria-label="Daily orders, retail versus social media"
      ...
      >
        ...
        {active && activeIndex !== null && width > 0 && (
          <div
            className="pointer-events-none absolute z-10 rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-md"
            style={{
              top: PAD.top,
              left: Math.min(
                Math.max(plot.x(activeIndex) + 12, PAD.left),
                width - 150,
              ),
            }}
          >
